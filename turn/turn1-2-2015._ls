...
;|
 ...
 I could have an associative key to the index at which is found each item (like a DB index).
 2015-10-22: I think I am pretty okay with having the lists not be assoociative.  I can build indexes on the fly.
 ...
 *WIKI-TURN-OBJECTS*
 ...
 2015: I think that we should not allow the drawing of shapes in a segment directly.  All shapes should be drawn as tblocks and inserted into tblocks or segments.  It's not good to have two shape-drawing animals.
...
;;;
 ...
 TBLOCK
 For reuse in segments.  Initially intended for wheels. Initially can't be scaled or rotated. Defined very much like a segment.  But you can't have the guide point, traling point, or hitch point in a tblock
...
(SETQ
  ;; Guide point. This is the steering axle or trailer tongue point of this vehicle segment.
  *WT-POINT-GUIDE*
   1
  ;; Trailing point. This is the back axle centroid point vehicle segment.
  *WT-POINT-TRAIL*
   2
  ;; Hitch point.  This is where the guide point of the trailing vehicle segment connects to this vehicle segment.
  *WT-POINT-HITCH*
   4
  ;; Path. Indicates that a path line should be plotted at this point. Layer required in point definition.
  *WT-POINT-PATH*
   8
  ;; Shape start. For use in vehicle or tblock definitions.  Layer goes in point definition.
  *WT-POINT-SHAPE-START*
   16
  ;; End shape open (For use in vehicle or tblock definitions.
  *WT-POINT-SHAPE-OPEN*
   32
  ;; End shape closed (For use in vehicle or tblock definitions.
  *WT-POINT-SHAPE-CLOSED*
   64                                  
  ;; Drawing block. Indicates that a drawing block should be inserted at this point. Name required.  Layer goes in point definition.
  *WT-POINT-DBLOCK*
   128
  ; Turn block. Indicates that a pre-defined turn block should be plotted at this point. Name required.  Layer goes in point definition.
  *WT-POINT-TBLOCK*
   256                                    
)
...
(DEFUN
  ...
  (LIST
    "segment"
    (CONS "id" index)
    (LIST "point" (LIST "position" 0 0) ("flags" *WT-POINT-GUIDE*))
    (LIST
      "point"
      (LIST "position" 20.0 PI)
      (CONS "flags" (+ *WT-POINT-PLOT-PATH* *WT-POINT-BACK-AXLE*))
      (CONS
        "pathlayer"
    ...
    (LIST
      ...
      (CONS "flags" (+ *WT-POINT-PLOT-PATH* *WT-POINT-HITCH*))
      (CONS
        "pathlayer"
    ...
    (LIST
      ...
      (CONS
        ("tblocklayer" . (STRCAT "C-TURN-SEG" (ITOA index) "-BODY"))
	("usename" . "stubwheel")
    ...
    (LIST
      ...
      (CONS "flags" *WT-POINT-TBLOCK*)
      (CONS
    ("blocklayer" . (STRCAT "C-TURN-SEG" (ITOA index) "-BODY"))
    ("usename" . "stubwheel")
      )
    )
  )
...
(DEFUN
  ...
  '("tblock"
    ...
    ("tblockpoint" ("position" 1 (* pi 0.25)) ("flags" *WT-POINT-SHAPE-START*))
    ("tblockpoint" ("position" 1 (* pi 0.75)))
    ("tblockpoint" ("position" 1 (* pi 1.25)))
    ("tblockpoint" ("position" 1 (* pi 1.75)) ("flags" *WT-POINT-SHAPE-CLOSED*))
   )
)
(DEFUN
   WIKI-TURN-STUB-STEPS ()
...
(DEFUN
   WIKI-TURN-DRAW-SHAPE (tblock vehicle insertion rotation parentlayer / layerkey lwpoly points SEGMENTPOLY)
  (FOREACH
    ...
    (SETQ
      layerkey
       (COND
         ...
         ((CDR (ASSOC "tblocklayer" (CDR tblockpoint))))
